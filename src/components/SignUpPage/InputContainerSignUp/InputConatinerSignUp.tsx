...
type InputConatinerProps = {
  ...
  setInputDataValue: (newValue: string, newCorrect: boolean) => void;
...
export const InputConatiner = ({
  ...
  if (content == 'Postal Code') {
    inputDataValue.setError = (newError: string) => {
  ...
  const handleChange = (value: string) => {
    ...
    if (patterns) {
      ...
      error = checkResult ? `*${checkResult.errorMessage}` : '';
      newCorrect = !error;
      ...
      if (birthdateThisYear > currentDate) age -= 1;

      if (age < 14) {
        error = '*Must be at least 14 years of age';
        newCorrect = false;
      }
    }

    setInputDataValue(value, newCorrect);
    setTextError(error);
  };
  ...
  return (
    <div
      className={`
        ...
        `}
    >
      <label className={classes.signUp__label}>{content}</label>
      ...
      <div className="text-red-500 text-xs h-3">{textError}</div>
    </div>
  );
};
